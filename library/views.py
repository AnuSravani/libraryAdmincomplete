...
#     if request.method == "POST":
#         if models.Book.objects.filter(accession_number=request.POST["bookname"]):
#             bookname=models.Book.objects.get(accession_number=request.POST["bookname"])
...
def bookytransaction_view(request):
    ...
    if request.method == "POST":
        if models.Book.objects.filter(accession_number=request.POST["bookname"]).exists():
            bookname = models.Book.objects.get(accession_number=request.POST["bookname"])
            member = models.addusermodel.objects.get(email=request.POST["email"])
            form = models.Transactionmodel.objects.create(
                book=bookname,
